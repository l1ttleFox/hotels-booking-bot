...
def start_collecting_hotel_info(message: Message) -> None:
    ...
    bot.send_message(message.from_user.id, f"{message.from_user.username}, где хочешь остановиться?")
...
def get_check_in_date(message: Message) -> None:
    ...
    try:
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            date = list()
            for i_elem in message.text.split("."):
        ...
        bot.send_message(message.from_user.id, "Я запомнил. Теперь введи дату выезда (гггг.мм.дд).")
...
def get_check_out_date(message: Message) -> None:
    ...
    try:
        ...
        bot.send_message(message.from_user.id, "Я запомнил. Теперь введи количество взрослых.")
...
def get_adults_amount(message: Message) -> None:
    ...
    bot.send_message(message.from_user.id, "Я запомнил. Теперь введи количество детей.")
